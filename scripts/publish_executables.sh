...
function package {
  ...
  cp ${BUILD_DIR}/${BINARY_DIR}/${BINARY_FILE} ${PACKAGE_STAGING_DIR}/autoeasy${FILE_EXTENSION}
...
do
  ...
  package_name="autoeasy-${GOOS}-${GOARCH}"
  ...
  package autoeasy-${VERSION}-${GOOS}-${GOARCH} ${package_name} ${FILE_EXTENSION}
